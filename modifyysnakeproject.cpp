...
int main()
{
    ...
    while(game)
    {
        setfillstyle(SOLID_FILL,BLUE); // initialization back ground color
        bar(0,0,660,500);  //  initializing background size or wide (lower width 630 and right length 470)

        setfillstyle(SOLID_FILL,BLACK);  // initializing boarder color

        bar(0,0,660,10); // upper boarder width,length

        bar(0,0,10,500); // right boarder width,length

        bar(0,500,660,490); // lower boarder width,length

        bar(650,10,660,500); // lower boarder width,length
        ...
        if(snakeX[0] == foodX && snakeY[0] == foodY) // checking snake and food is in the same position or not
        {
            length = length + 1;
            food = food +1 ;
            delayTime = delayTime - 10;  // food consumed by snake then the delay time reduced and snake spread increase
